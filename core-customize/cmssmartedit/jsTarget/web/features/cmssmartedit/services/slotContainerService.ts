 ...
 */
@SeInjectable()
export class /* @ngInject */ SlotContainerService {
    // --------------------------------------------------------------------------------------
    // Variables
    // --------------------------------------------------------------------------------------
    ...
    public getComponentContainer(
    ...
    ): angular.IPromise<ContainerInfo> {
        return this.loadContainersInPageInfo().then((containersInPage: ContainerInfo[]) => {
            const containers = containersInPage.filter((container: ContainerInfo) => {
                return (
                    container.slotId === slotId &&
                    this.lodash.includes(container.components, componentUuid)
                );
            });
    ...
    private loadContainersInPageInfo() {
        if (this.containersInPage) {
            ...
            const promise = this.experienceService
