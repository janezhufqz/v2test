 ...
 */
@SeInjectable()
export class /* @ngInject */ TypePermissionsRestService {
     ...
     * @returns {Promise<TypedMap<boolean>>} A promise that resolves to a TypedMap object with key (the code) and
     ...
     *
     * @param {String[]} types The codes of all types.
     * @returns {Promise<TypedMap<TypedMap<boolean>>>} A promise that resolves to a TypedMap of TypedMap object with key (the code) and
    ...
    hasAllPermissionsForTypes(types: string[]): Promise<TypedMap<TypedMap<boolean>>> {
        ...
        return this._getAllPermissionsForTypes(types).then(
                ...
                return response.reduce(
                    (
                        ...
                        if (permissionsResult.permissions) {
                            ...
                            ] = this._getPermissionByNameAndResult(
                                permissionsResult,
                                TypePermissionNames.READ
                            );
                            map[permissionsResult.id][
    ...
    private _getPermissionByNameAndResult(
        permissionsResult: IPermissionsRestServiceResult,
    ...
    private _getPermissionsForTypesAndName(
        types: string[],
        permissionName: string
            ...
            (response: IPermissionsRestServiceResult[]) => {
                return response.reduce(
                    (map: TypedMap<boolean>, permissionsResult: IPermissionsRestServiceResult) => {
                        if (permissionsResult.permissions) {
                            map[permissionsResult.id] = this._getPermissionByNameAndResult(
                                permissionsResult,
                                permissionName
                            );
                        }
    ...
    private _getAllPermissionsForTypes(types: string[]): Promise<IPermissionsRestServiceResult[]> {
        if (types.length <= 0) {
