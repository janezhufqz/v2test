 ...
 */
@SeInjectable()
export class /* @ngInject */ RollbackPageVersionService {
    constructor(
    ...
    rollbackPageVersion(version?: IPageVersion): void {
        ...
        if (!!pageVersion) {
            ...
            this.pageInfoService.getPageUUID().then((pageUuid: string) => {
                this.showConfirmationModal(pageVersion.label, TRANSLATE_NS).then(
                    this.performRollback.bind(this, pageUuid, pageVersion)
                );
            });
        }
    }
