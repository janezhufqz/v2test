 ...
 */
@SeInjectable()
export class /* @ngInject */ PageVersionSelectionService {
    // ------------------------------------------------------------------------
    // Properties
    // ------------------------------------------------------------------------
    ...
    constructor(
        ...
        this.crossFrameEventService.subscribe(
            ...
            this.removePageVersionOnPerspectiveChange.bind(this)
        );
        this.crossFrameEventService.subscribe(
            EVENT_PERSPECTIVE_REFRESHED,
            this.resetPageVersionContext.bind(this)
        );

        // Required especially when a version is selected and you refresh the browser.
        this.crossFrameEventService.subscribe(
            this.EVENTS.PAGE_CHANGE,
            (eventId: string, experience: IExperience) => {
                if (experience.versionId && !this.selectedPageVersion) {
                    this.pageInfoService.getPageUUID().then((pageUuid: string) => {
                        this.pageVersioningService
                            .getPageVersionForId(pageUuid, String(experience.versionId))
                            .then((version: IPageVersion) => {
    ...
    public hideToolbarContextIfNotNeeded(): void {
        if (!this.selectedPageVersion) {
            this.crossFrameEventService.publish(
                this.HIDE_TOOLBAR_ITEM_CONTEXT,
    ...
    public showToolbarContextIfNeeded(): void {
        if (this.selectedPageVersion) {
            this.crossFrameEventService.publish(
                this.SHOW_TOOLBAR_ITEM_CONTEXT,
                this.PAGE_VERSIONS_TOOLBAR_ITEM_KEY
            );
        }
    }
        ...
        this.cMSModesService
            ...
            .then((isVersioningModeActive: boolean) => {
                if (this.selectedPageVersion) {
                    this.pageInfoService.getPageUUID().then((pageUuid: string) => {
                        if (
                            !isVersioningModeActive ||
                            this.selectedPageVersion.itemUUID !== pageUuid
                        ) {
