 ...
 */
@SeInjectable()
export class /* @ngInject */ PageVersioningService {
    // ------------------------------------------------------------------------
    // Properties
    // ------------------------------------------------------------------------
    ...
    constructor(
        ...
        this.pageVersionsServiceResourceURI = `/cmswebservices/v1/sites/${
            this.PAGE_CONTEXT_SITE_ID
        }/cmsitems/:pageUuid/versions`;
        this.pageVersionsRollbackServiceResourceURI = `/cmswebservices/v1/sites/${
            this.PAGE_CONTEXT_SITE_ID
        }/cmsitems/:pageUuid/versions/:versionId/rollbacks`;
     ...
     */
    public findPageVersions(
        payload: PageVersionSearchPayload
    ): angular.IPromise<Page<IPageVersion>> {
        return this.$q.when(this.pageVersionRESTService.page(payload));
    }
     ...
     */
    public getPageVersionForId(
        pageUuid: string,
        versionId: string
    ): angular.IPromise<IPageVersion> {
        return this.$q.when(
            this.pageVersionRESTService.get({
                pageUuid,
    ...
    public deletePageVersion(pageUuid: string, versionId: string): angular.IPromise<void> {
        return this.$q.when(
            this.pageVersionRESTService.remove({
                pageUuid,
                identifier: versionId
            })
        );
    ...
    public rollbackPageVersion(pageUuid: string, versionId: string): angular.IPromise<void> {
        return this.$q.when(this.pageVersionsRollbackRESTService.save({ pageUuid, versionId }));
    }
}
