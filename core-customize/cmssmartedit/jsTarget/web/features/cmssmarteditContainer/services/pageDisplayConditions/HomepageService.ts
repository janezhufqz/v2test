 ...
 */
export const DEFAULT_CMS_EVENT_SHOW_REPLACE_PARENT_HOMEPAGE_INFO =
 ...
 */
export const DEFAULT_CMS_EVENT_HIDE_REPLACE_PARENT_HOMEPAGE_INFO =
 ...
 */
@SeInjectable()
export class /* @ngInject */ HomepageService {
    constructor(
    ...
    ): angular.IPromise<ICatalogHomepageDetails> {
        const promise = this.catalogService
            .getContentCatalogVersion(uriContext)
            .then((catalogVersion: ICatalogVersion) => {
                ...
                if (
                    ...
                    return this.compareCatalogVersions<HomepageType>(
                        ...
                        (oldHomepageCatalog: ICatalogVersion, pageCatalog: ICatalogVersion) => {
                            return oldHomepageCatalog.catalogId === pageCatalog.catalogId
                                ? HomepageType.OLD
                                : null;
                        }
                    );
                } else if (
                    catalog.homepage.fallback &&
                    catalog.homepage.fallback.uid === cmsPage.uid
                ) {
                    return this.compareCatalogVersions<HomepageType>(
                        catalog.homepage.fallback.catalogVersionUuid,
                        cmsPage.catalogVersion,
                        (fallbackCatalog: ICatalogVersion, pageCatalog: ICatalogVersion) => {
                            return fallbackCatalog.uuid === pageCatalog.uuid
                                ? HomepageType.FALLBACK
                                : null;
                        }
                    );
                }
    ...
    canSyncHomepage(cmsPage: ICMSPage, uriContext: IUriContext): angular.IPromise<boolean> {
        return this.isOldHomepage(cmsPage, uriContext).then((isOld: boolean) => {
            if (!isOld) {
    ...
    private compareCatalogVersions<T>(
        catalogAUuid: string,
        catalogBUuid: string,
        compare: (a: ICatalogVersion, b: ICatalogVersion) => T
    ): angular.IPromise<T> {
        const promise = this.catalogService
            .getCatalogVersionByUuid(catalogAUuid)
            .then((catalogVersionA: ICatalogVersion) => {
                return this.catalogService
                    .getCatalogVersionByUuid(catalogBUuid)
                    .then((catalogVersionB: ICatalogVersion) => {
    ...
    private buildHomepageDetailsForContext(
        ...
        const promise = this.catalogService
            .getCatalogVersionByUuid(homepageForCurrentCatalog.catalogVersionUuid)
            .then((homepageCatalogVersion: ICatalogVersion) => {
                const homepageComesFromParent =
                    homepageCatalogVersion.catalogId !== uriContext[this.CONTEXT_CATALOG];
                return this.catalogService
                    .getContentCatalogVersion(uriContext)
                    .then((currentCatalogVersion: ICatalogVersion) => {
