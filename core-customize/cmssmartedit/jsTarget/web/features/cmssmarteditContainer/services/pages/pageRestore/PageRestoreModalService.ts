...
@SeInjectable()
export class /* @ngInject */ PageRestoreModalService {
    // ---------------------------------------------------------------------------
    // Variables
    // ---------------------------------------------------------------------------
    ...
    public handleRestoreValidationErrors(pageInfo: ICMSPage, errors: ValidationError[]): void {
        ...
        const unsupportedErrors: ValidationError[] = this.lodash.difference(
            errors,
            actionableErrors,
            nonActionableErrors
        );
        ...
        return this.genericEditorModalService.open(
            ...
            (newErrors: ValidationError[], ge: any) => {
                ...
                if (unsupportedErrors.length > 0) {
                    ...
                    structureFields = this.lodash.concat(
                        structureFields,
    ...
    private getRestoreEditorStructureFields(errors: ValidationError[]): any {
        return errors.reduce((accumulator: any[], currentError) => {
            const errorMapping = this.ACTIONABLE_ERRORS.find(
    ...
    private showConfirmationMessage(errors: ValidationError[]): void {
        const message = errors.reduce((accumulator, currentError) => {
            const errorMapping = this.NON_ACTIONABLE_ERRORS.find(
    ...
    private getNonActionableError(errors: ValidationError[]): ValidationError[] {
        return errors.filter((error) => {
            return this.NON_ACTIONABLE_ERRORS.some(
                (nonActionableErrorMapping: NonActionableErrorMapping) => {
                    return (
                        this.isSupportedError(error) &&
                        this.isEqualTo(error, nonActionableErrorMapping)
                    );
                }
            );
        });
