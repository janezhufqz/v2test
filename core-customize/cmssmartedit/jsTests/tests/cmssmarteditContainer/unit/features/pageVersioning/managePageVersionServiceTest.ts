...
describe('Test ManagePageVersionService', () => {
    ...
    const promisePageUUID = promiseHelper.buildPromise<string>(
        'promisePageUUID',
        PromiseType.RESOLVES,
        PAGE_UUID
    );
    const promisePageVersion = promiseHelper.buildPromise<IPageVersion>(
        'promisePageVersion',
        PromiseType.RESOLVES,
        PAGE_VERSION
    );
    ...
    beforeEach(() => {
        ...
        pageVersionSelectionService = jasmine.createSpyObj<PageVersionSelectionService>(
            ...
            ['selectPageVersion', 'getSelectedPageVersion', 'deselectPageVersion']
        );

        pageVersioningService.getResourceURI.and.returnValue(RESOURCE_URI);

        managePageVersionService = new ManagePageVersionService(
            alertService,
            experienceService,
