 ...
 */
@SeInjectable()
export class /* @ngInject */ HiddenComponentMenuService {
    // --------------------------------------------------------------------------------------
    // Constants
    // --------------------------------------------------------------------------------------
    ...
    constructor(
        ...
        private contextualMenuService: any,
     ...
     * provided.
     */
    public getItemsForHiddenComponent(
        component: any,
        slotId: string
    ): angular.IPromise<IHiddenComponentMenu> {
        this._validateComponent(component);

        return this._buildComponentInfo(slotId, component).then(
    ...
    private _buildComponentInfo(
        slotId: string,
        component: any
    ): angular.IPromise<IContextualMenuConfiguration> {
        return this.slotContainerService
            .getComponentContainer(slotId, component.uuid)
    ...
    private _getAllowedItemsForComponent(
        component: any,
        configuration: IContextualMenuConfiguration
    ): angular.IPromise<IHiddenComponentMenu> {
        ...
        menuItems
            .filter((item: IContextualMenuButton) => {
            ...
            .forEach((item: IContextualMenuButton) => {
                ...
                this.$q
                    .when(item.condition ? item.condition(configuration) : true)
