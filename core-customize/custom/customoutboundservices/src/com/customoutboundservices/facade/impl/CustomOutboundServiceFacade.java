 ...
 */
package com.customoutboundservices.facade.impl;
...
/**
 * Custom implementation of OutboundServiceFacade.
 */
public class CustomOutboundServiceFacade implements OutboundServiceFacade
{
	private static final Logger LOG = Log.getLogger(CustomOutboundServiceFacade.class);
	...
	private void logRequestIfMonitoringIsEnabled(final ItemModel itemModel, final String integrationObjectCode,
			final ConsumedDestinationModel destination, final Map<String, Object> payload)
	{
	...
	protected Observable<ResponseEntity<Map>> createObservable(final RestOperations restOperations, final ItemModel itemModel,
			final String integrationObjectCode, final ConsumedDestinationModel destinationModel, final Map<String, Object> payload)
	{
		return Observable.just(restOperations).map(restTemplate -> {
			final HttpEntity<Map<String, Object>> entity = createHttpEntity(itemModel, integrationObjectCode, destinationModel,
					payload);
			LOG.debug("HttpEntity map:", entity);
			return restTemplate.postForEntity(destinationModel.getUrl(), entity, Map.class);
		});
	}

	protected HttpEntity<Map<String, Object>> createHttpEntity(final ItemModel itemModel, final String integrationObjectCode,
			final ConsumedDestinationModel destinationModel, final Map<String, Object> payload)
	{
		final String integrationObjectItemCode = findIntegrationObjectItemCode(integrationObjectCode, itemModel);

		final HttpHeaders httpHeaders = new HttpHeaders();
		final DecoratorContext context = DecoratorContext.decoratorContextBuilder().withDestinationModel(destinationModel)
				.withIntegrationObjectCode(integrationObjectCode).withIntegrationObjectItemCode(integrationObjectItemCode)
				.withItemModel(itemModel).build();
	...
	protected String findIntegrationObjectItemCode(final String integrationObjectCode, final ItemModel itemModel)
	{
		try
		{
			return getIntegrationObjectService().findIntegrationObjectItemByTypeCode(integrationObjectCode, itemModel.getItemtype())
					.getCode();
		}
